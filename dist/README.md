...
0.3.0 是一次前端升级 —— 面板可以按你的喜好来了：

- **主题色可选** —— 在 **高级设置 → 主题色** 里从七种精选配色中挑（哔哩蓝、青碧、翠绿、星紫、少女粉、落日橙、石墨灰）。默认仍是原来的哔哩蓝，不选就不变。
- **浅色 / 深色开关** —— 面板顶部新增一个日 / 月开关，终于有了正经的深色模式，不再是深色页面上一块刺眼的白框。默认跟随系统，点一下后记住你的选择。
- **底层重构** —— 所有颜色都走一层轻量 token，主题色与明暗背景在状态区、实时速度图、开关和 ⚡ 图标之间保持一致。
